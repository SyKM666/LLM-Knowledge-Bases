...
| 环境变量 | 说明 | 默认值 |
|---------|------|--------|
| `KB_LLM_PROVIDER` | `claude-cli` / `openai` / `anthropic` | `claude-cli` |
| `KB_LLM_MODEL` | 模型名称 | 按 provider 而定 |
| `KB_LLM_BASE_URL` | 自定义 API 地址（用于本地模型） | — |
| `KB_LLM_API_KEY` | API 密钥（也读取 `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`） | — |

## 工作流程

1. **摄入** — 将文档导入 `raw/` 目录
2. **编译** — LLM 将原始文档转换为带标题、分类、内链的结构化 Wiki 文章，存入 `wiki/`
3. **索引** — 自动生成 `wiki/INDEX.md` 分类目录
